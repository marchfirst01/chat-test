...
export const socketJoin = (socket, io, { name, room }, callback) => {
  ...
  console.log('join_u', user);
  console.log('join_ub', user_bef);

  socket.join(room); // 사용자가 room에 입장할 수 있도록 함

  socket.emit('message', {
    // message event가 요청(발생) 되면 아래 내용 전달
    user: 'admin',
    text: `${name}, ${room}에 오신 것을 환영합니다.`,
  });

  io.to(room).emit('roomData', {
    // 그리고 해당 방의 모든 사용자들에게 roomData 전달 -> 실시간 업데이트 가능(추후 읽음 표시로 사용 가능할 듯, 1에서 0으로 없어지는거)
    room: room,
